...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $noteContent = trim($_POST['content']);

        if (empty($courseName) || empty($topicName) || empty($noteContent)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            $typeUploadDir = $baseUploadDir . DIRECTORY_SEPARATOR . 'notes';
            ...
            $newFilename = 'note_' . uniqid() . '.' . $extension;
            ...
            $imagePath = 'uploads/notes/' . $newFilename;
        }
        ...
        // Insert note with image path
        $stmt = $conn->prepare("INSERT INTO notes (userId, topicId, noteText, image_path, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())");
        $stmt->bind_param("iiss", $userId, $topicId, $noteContent, $imagePath);
        ...
        error_log("Error in noteAdd.php: " . $e->getMessage());
